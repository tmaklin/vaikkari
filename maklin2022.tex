...
Assuming that the reads $r_{n}$ are independent of each other, $r_{j} \indept r_{i}$ for all $i \neq j, 0 < i < N, 0 < j < N$, the model factorizes into
\begin{equation}
...
\begin{proof}
  Follows from $\Gamma\left(z + 1\right) = z\Gamma\left(z\right), \text{ for all } z > 0$ \citep{davis_leonhard} and $B\left(a, b\right) = \frac{\Gamma\left(a\right)\Gamma\left(b\right)}{\Gamma\left(a + b\right)}$ \citep{artin_einfuhrung} through
  \begin{align*}
