...
Mixture models have been utilized for several decades in modelling
sequencing data originating from different sources. One of the
earliest mentions of using such a model is found in the context of
.... (TODO cite the paper Antti showed), where ... .

With the introduction of faster methods for sequence alignment and
more accurate sequencing reads, the model from (TODO cite above) was
eventually extended with more advanced likelihood functions to deal
with .... (cite the developments). This ....

%%%%
- write about Reference data vs Reference-free approach <- might go to intro?

While the above models deal with an approach where sequence alignment
against reference data is used to evaluate the compatibility between
the reads and the reference, some alternative reference-free
approaches have also been developed. Typically based on using
distances between the sequencing reads, these methods (TODO citations).
...
\section{Sequence alignment and reference databases}
...
and other genomic analyses. In the same spirot, metagenomic
...
results obtained using only abundace estimates are in line with the
