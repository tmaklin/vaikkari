...
chapters (\ref{mixture-modelling-of-sequence-data}) describes the
...
(\ref{high-resolution-metagenomics}) describes the experimental
...
chapter (\ref{section:metagenomic-epidemiology})is more speculative in
...
chapter (\ref{conclusions-and-future-directions}) followed by reprints
...
\noindent\let\thefootnote\relax\footnote{$^{\star}$ \url{https://github.com/tseemann/shovill}}
...
shovill$^{\star}$, which is an assembly pipeline built around the SPAdes
...
\noindent\let\thefootnote\relax\footnote{Figure \ref{fig:mgems-assembler-choice-statistics} source: Adapted from Publication II, Figure 3 \citep{maklin_bacterial_2021}.}
\addtocounter{footnote}{-1}\let\thefootnote\svthefootnote
\begin{figure}[!ht]
  \label{fig:mgems-assembler-choice-statistics}
    ...
    \caption{Comparing mGEMS-derived assemblies with different
      assembler choices. The boxes are colored according to the type
      of assembly. The presented statistics are the summed lengths of
      all contigs (total length), the number of contigs, the sequence
      length of the shortest contig at 50\% genome length (N50), and
      the smallest number of contigs whose sum of lengths is at least
      50\% of the genome length (L50).}
\end{figure}


\subsubsection{Quality control}

\noindent\let\thefootnote\relax\footnote{$^{\star}$ \url{https://github.com/harry-thorpe/demix_check}}
...
QC step, called demix\_check$^{\star}$, performs several checks on the results from mSWEEP and
...
\noindent\let\thefootnote\relax\footnote{$^{\star}$ \url{https://github.com/tseemann/snippy}}
...
snippy$^{\star}$ with the same reference genome for both mGEMS-derived
...
(Figure \ref{fig:mgems-ecoli-snps}) but the phylogenetic relationships (Figure \ref{fig:mgems-ecoli-snps}) are recovered well. This benchmark was
...
\noindent\let\thefootnote\relax\footnote{Figure \ref{fig:mgems-ecoli-snps} source: Adapted from Publication II, Figure 3 \citep{maklin_bacterial_2021}.}
\addtocounter{footnote}{-1}\let\thefootnote\svthefootnote
\begin{figure}[!t]
  \label{fig:mgems-ecoli-snps}
    ...
    \caption{Comparing the results of SNP calling from mixed samples
      with the mGEMS pipeline against the results from isolate
      sequencing data for \textit{E. coli} ST131. SNPs were called
      from contigs after assembling the reads. Points are colored
      according to the lineage within the species (the full legend is
      available in Fig. S3). The dashed gray line represents a
      hypothetical perfect match between the binned and isolate
      reads. The blue line is the posterior mean while the shaded area
      contains the 95\% posterior credible region calculated from
      10000 posterior samples from a Bayesian regression model with
      the SNPs from the binned reads as the response and the SNPs from
      the isolate sequencing data as the sole explanatory variable.}
\end{figure}
\noindent\let\thefootnote\relax\footnote{Figure \ref{fig:mgems-ecoli-phylogeny} source: Reproduced from Publication II, Figure 4 \citep{maklin_bacterial_2021}.}
\addtocounter{footnote}{-1}\let\thefootnote\svthefootnote
\begin{figure}[!t]
  \label{fig:mgems-ecoli-phylogeny}
    ...
    \caption{Midpoint-rooted maximum likelihood trees from core SNP alignment of E. coli ST131 strains. The phylogeny in panel a was constructed from isolate sequencing data from 30 E. coli ST131 strains, and the phylogeny in panel b with the mGEMS pipeline from ten synthetic plate sweep samples, each mixing three isolate samples from the three main ST131 lineages (a–c; one strain from each per sample).}
\end{figure}
...
between the two.
\noindent\let\thefootnote\relax\footnote{Figure \ref{fig:mgems-ecoli-phylogeny} source: Reproduced from Publication II, Figure 5 \citep{maklin_bacterial_2021}.}
\addtocounter{footnote}{-1}\let\thefootnote\svthefootnote
\begin{figure}[!t]
  \label{fig:mgems-efaecalis-phylogeny}
    ...
    \caption{Tanglegram of two midpoint-rooted maximum likelihood
      trees from core SNP alignment of E. faecalis strains. The
      phylogenyies were inferred with RAxML-NG. Numbers below the
      edges indicate bootstrap support values from RAxML-NG for the
      next branch towards the leaves of the tree. Branches are
...
assemblies.
\noindent\let\thefootnote\relax\footnote{Figure \ref{fig:mgems-ecoli-phylogeny} source: Reproduced from Publication II, Figure 6 \citep{maklin_bacterial_2021}.}
\addtocounter{footnote}{-1}\let\thefootnote\svthefootnote
\begin{figure}[!t]
  \label{fig:mgems-saureus-phylogeny}
    ...
    \caption{Midpoint-rooted maximum likelihood tree from core SNP
      alignment of S. aureus ST22 showing strains from a single
      lineage within the sequence type. The phylogeny was inferred
      from a combined set of assemblies from 60 isolate sequencing
      samples (leaves labelled Staff A-G 1 A-T, corresponding to the
      temporally first samples from each staff member) and 312
      assemblies obtained from the mGEMS pipeline applied to synthetic
      mixed samples of sequencing data from each of the three
      different S. aureus ST22 clades (1, 2, and 3).}
\end{figure}
...
and virulence factors in them. This analysis somewhat surprisingly
found no significant differences between the vaginally delivered and
...
WGS metagenomics data, plate sweeps could be incorporated into the
...
be enrichened for interesting species.

The introduction of mSWEEP and mGEMS facilitates these types of
analyses and other research directions. Consideration of strain-level
